...
park_border <- boundary %>%
  sf::st_cast(., "MULTILINESTRING") #takes orginal polygon and turns it into a line

#can break polygon into lines or even points via st_cast
```
...
```{r}
polygon_raster <- terra::rasterize(park_polygon, elevation) #elevation is template raster, using grid cells but data 
border_raster <- terra::rasterize(park_border, elevation) #again elevation is template raster
```

```{r}
tm_shape(polygon_raster) +
  tm_raster() #only one layer, hence description of one layer

tm_shape(border_raster) +
  tm_raster() #polygon vs line you are getting different types of raster grids, whatever interests with the geometry counts 
```
